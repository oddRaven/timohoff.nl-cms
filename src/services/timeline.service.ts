...
export class TimelineService {
    private axiosInstance : AxiosInstance = axios.create({
        baseURL: `${import.meta.env.VITE_API_URL}/timeline`
    });
    ...
    public async getAll(): Promise<any> {
        ...
        return await this.axiosInstance.get<ITimeline[]>(path, this.config);
    }
    ...
    public async put(id: number, data: ITimeline) : Promise<any> {
        const path = '/' + id;
    ...
    public async post(data: ITimeline) : Promise<any> {
        const path = '';
        this.config.withCredentials = true;
